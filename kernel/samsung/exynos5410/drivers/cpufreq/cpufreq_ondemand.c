...
static bool boot_delay[4] = { true, true, true, true};
...
static ssize_t store_boost_mode(struct kobject *a, struct attribute *b,
				  const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	dbs_tuners_ins.boost_mode = input ? true : false;
	return count;
}
