...
#define SSP_FIRMWARE_REVISION_08	15120300/* MCU L5 */
...
{
		return SSP_FIRMWARE_REVISION_08;
}
...
void toggle_mcu_reset(struct ssp_data *data)
{
	...
	data->client->adapter->nak_timeout = msecs_to_jiffies(10);
	...
	data->client->adapter->nak_timeout = msecs_to_jiffies(1000);
}
...
int check_fwbl(struct ssp_data *data)
{
	...
	if (data->uCurFirmRev == SSP_INVALID_REVISION) {
		...
		if (iRet >= 0) {
			...
			} else if (data->uCurFirmRev != SSP_FIRMWARE_REVISION_08) {
				pr_info("[SSP] MCU Firm Rev : Old = %8u, New = %8u\n",
					data->uCurFirmRev, SSP_FIRMWARE_REVISION_08);
		...
		if (data->uCurFirmRev != SSP_FIRMWARE_REVISION_08) {
			pr_info("[SSP] MCU Firm Rev : Old = %8u, New = %8u\n",
				data->uCurFirmRev, SSP_FIRMWARE_REVISION_08);
